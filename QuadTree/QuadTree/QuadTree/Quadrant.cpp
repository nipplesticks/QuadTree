...
void Quadrant::Create(const DM::Vec2f & pos, float size, unsigned int currentLevel, bool isLeaf)
{
	m_min = pos;
	m_max = pos + DM::Vec2f(size, size);
	m_size = size;
...
const DM::Vec2f & Quadrant::GetMin() const
{
	return m_min;
}

const DM::Vec2f & Quadrant::GetMax() const
{
	return m_max;
}
